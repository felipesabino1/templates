...
struct SA{
	...
	void rec(vector<int>& v, int k) {
		...
		for (int i = 0; i < sz2; i++) {
			if (v[tmp[i]] != l0 or v[tmp[i]+1] != l1 or v[tmp[i]+2] != l2)
				l0 = v[tmp[i]], l1 = v[tmp[i]+1], l2 = v[tmp[i]+2], diff++;
			if (tmp[i]%3 == 1) R[tmp[i]/3] = diff;
			else R[tmp[i]/3+sz] = diff;
		}
	...
	SA(const string& ss) : s(ss), n(s.size()), sa(n+3),
			cnt(n+1), pos(n), lcp(n-1) {
		vector<int> v(n+3);
		for (int i = 0; i < n; i++) v[i] = i;
		radix(&v[0], &pos[0], &s[0], n, 256);
		int diff = 1;
		for (int i = 0; i < n; i++)
			v[pos[i]] = diff += (i and s[pos[i]] != s[pos[i-1]]);
		if (n >= 2) rec(v, diff);
		sa.resize(n);
    ...
    i = pos[i], j = pos[j];
    return (i == j ? n-i : RMQ.query(min(i,j),max(i,j)-1));
	}
};
